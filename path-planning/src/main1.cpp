          ...
          // j[1] is the data JSON object
            ...
            for (int i=0; i<sensor_fusion.size(); i++)
            {
              ...
              if (d < 2 + 4*lane + 2 && d > 2 + 4*lane -2) // another car in my lane
              {
                ...
                // extrapolate (s, d) one time step in the future
                // x0 = sensor_fusion[i][1];
                // y0 = sensor_fusion[i][2];
                d0 = sensor_fusion[i][6];
                //
                // x1 = x0 + vx*0.02;
                // y1 = y0 + vy*0.02;
                //
                // vector<double> new_frenet = getFrenet(x1, y1, ref_yaw, map_waypoints_x, map_waypoints_y);
                //
                // s1 = new_frenet[0];
                // d1 = new_frenet[1];
                //
                // vs = (s1-check_car_s)/0.02;
                // vd = (d1-d0)/0.02;
                //
                // vector<double> coords;
                // coords.push_back(check_car_s);
                // coords.push_back(d0);
                // coords.push_back(vx);
                // coords.push_back(vy);
                //
                ...
                // int c_w = ClosestWaypoint(x0, y0, map_waypoints_x, map_waypoints_y);
                // int n_w = NextWaypoint(x0, y0, ref_yaw, map_waypoints_x, map_waypoints_y);
                // double wp1_x = map_waypoints_x[n_w];
                // double wp1_y = map_waypoints_y[n_w];
                // double wp0_x = map_waypoints_x[c_w];
                // double wp0_y = map_waypoints_y[c_w];
                //
                // double car_heading  = atan(vy/vx);
                // double lane_heading = atan((wp1_y - wp0_y) / (wp1_x - wp0_x));
                // double delta_theta = car_heading - lane_heading;
                //
                // double mag_v = sqrt(pow(vx,2) + pow(vy,2));
                //
                // double vs = mag_v * cos(delta_theta);
                // double vd = mag_v * sin(delta_theta);

                // if the car is too close (less than 30 meters)
                if (check_car_s > car_s && check_car_s - car_s < 30)
                {
                  too_close = true; // change proximity flag
                  front_car = check_speed;
                  // vector<double> coords;
                  // coords.push_back(check_car_s);
                  // coords.push_back(d0);
                  // coords.push_back(vs);
            ...
            if (too_close && ref_vel>front_car)
            {
