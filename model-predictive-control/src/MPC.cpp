...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += CppAD::pow(vars[cte_start + t], 2)*1e3;
      fg[0] += CppAD::pow(vars[epsi_start + t], 2)*1e3;
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> x02 = x0*x0;
      AD<double> f0 = coeffs[0] + coeffs[1]*x0 + coeffs[2]*x02 + coeffs[3]*x0*x02;
      AD<double> psides0 = CppAD::atan(3*coeffs[3]*x02 + 2*coeffs[2]*x0 + coeffs[1]);
